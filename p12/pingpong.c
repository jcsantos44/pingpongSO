...
int mqueue_create (mqueue_t *queue, int max, int size)
{
	...
	queue->fila_msg = malloc(max*size);
...
int mqueue_send (mqueue_t *queue, void *msg){
	...
	#endif
	dest  = &queue->fila_msg[i];//endereco da variavel de destino da mensagem
	...
	queue->write_pointer = (queue->write_pointer+1) % queue->maxmsg;
...
int mqueue_recv (mqueue_t *queue, void *msg)
{
	...
	orig = &(queue->fila_msg[i]); //endereco da variavel de origem da mensagem
	memcpy(dest,orig,queue->size);//copia queue->size bytes da mensagem em orig para o dest
